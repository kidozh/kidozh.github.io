...
 [view plain](http://blog.csdn.net/wallwind/article/details/6899330 "view plain")[copy](http://blog.csdn.net/wallwind/article/details/6899330 "copy")
...
1.  5.   **    for** ( i = 1; i <1000; i ++ ) printf("This is child process/n");   
...
1.  9.   **        for** ( i = 1; i <1000; i ++ ) printf("This is process process/n");   
...
1.  8.           fgets( command, 256, stdin );   



1.  9.           command[strlen(command)-1] = 0;   
...
1.  12.              execlp( command, command );   



1.  13.          /* 如果exec函数返回，表明没有正常执行命令，打印错误信息*/   
...
1.  20.              printf( " child process return %d/n",. rtn );   
...
1.  19.           printf("in the spawned (child) process.../n");   



1.  20.           /*子进程向父进程写数据，关闭管道的读端*/   



1.  21.           close(file_descriptors[INPUT]);   



1.  22.           write(file_descriptors[OUTPUT], "test data", strlen("test data"));   
...
1.  27.       printf("in the spawning (parent) process.../n");   



1.  28.       /*父进程从管道读取子进程写的数据，关闭管道的写端*/   



1.  29.       close(file_descriptors[OUTPUT]);   



1.  30.       returned_count = read(file_descriptors[INPUT], buf, **sizeof**(buf));   



1.  31.       printf("%d bytes of data received from spawned process: %s/n",   



1.  32.       returned_count, buf);   
...
1.  13.  **     while** ((count = fread(buf, 1, 80, in_file)) > 0)   



1.  14.           printf("received from pipe: %s/n", buf);   
...
1.  24.       out_file = fopen("mypipe", "w");   



1.  25.  **     if** (out_file == NULL) {   



1.  26.       printf("Error opening pipe.");   
...
1.  29.       sprintf(buf,"this is test data for the named pipe example/n");   



1.  30.       fwrite(buf, 1, 80, out_file);   



1.  31.       fclose(out_file);   
...
1.  1.   #include <sys /types.h>   </sys>



1.  2.   　　 #include <sys /ipc.h>   </sys>



1.  3.   　　 #include <sys /sem.h>   </sys>



1.  4.   　　 **int** semget(key_t key, **int** nsems, **int** flag);   
...
1.  3.   #include <sys/sem.h>   



1.  4.   #include <sys/ipc.h>   
...
1.  6.   key_t unique_key; /* 定义一个IPC关键字*/   
...
1.  8.   **struct** sembuf lock_it;   



1.  9.   **union** semun options;   
...
1.  12.  unique_key = ftok(".", 'a'); /* 生成关键字，字符'a'是一个随机种子*/   
...
1.  14.  id = semget(unique_key, 1, IPC_CREAT | IPC_EXCL | 0666);   



1.  15.  printf("semaphore id=%d/n", id);   



1.  16.  options.val = 1; /*设置变量值*/   



1.  17.  semctl(id, 0, SETVAL, options); /*设置索引0的信号量*/   
...
1.  20.  i = semctl(id, 0, GETVAL, 0);   



1.  21.  printf("value of semaphore at index 0 is %d/n", i);   
...
1.  24.  lock_it.sem_num = 0; /*设置哪个信号量*/   



1.  25.  lock_it.sem_op = -1; /*定义操作*/   



1.  26.  lock_it.sem_flg = IPC_NOWAIT; /*操作方式*/   



1.  27.  **if** (semop(id, &lock_it, 1) == -1) {   



1.  28.  printf("can not lock semaphore./n");   
...
1.  32.  i = semctl(id, 0, GETVAL, 0);   



1.  33.  printf("value of semaphore at index 0 is %d/n", i);   
...
1.  2.   **DWORD** WINAPI ChildProcess( **LPVOID** lpParameter ){   
...
1.  6.   printf( "This is Child Thread: %d/n", g );   
...
1.  16.  CreateThread( NULL, 0, ChildProcess, NULL, 0, &threadID );   
...
1.  19.  printf( "This is Parent Thread: %d/n", g );   
...
         4 鸣谢
　　       本文部分内容参照[www.lisoleg.org](http://www.lisoleg.org/)内的《Linux下的多进程编程》，原作者俞磊。




         The part ends in there.<<
...
1.  15.  　　   ret=pthread_create(&id,NULL,(**void** *) **thread**,NULL);   
...
1.  17.  　　   printf ("Create pthread error!\n");   
...
1.  21.  　　   printf("This is the main process.\n");   



1.  22.  　　   pthread_join(id,NULL);   
